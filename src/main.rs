...
use core::cap::{spawn_actor, Trans};
use core::item;
...
fn main() {
    ...
    let mut item = spawn_actor(item_data);
    range(0u, NUM_TASKS).map( |_| {
        static ITEMS_PER_TASK: uint = NUM_ITEMS / NUM_TASKS;
        Future::spawn(proc() {
            let mut item = spawn_actor(Item::make(ITEM_DATA));
            for _ in range(0u, ITEMS_PER_TASK) {
                let new_item = spawn_actor(Item::make(ItemData));
    ...
    let mut mobcap = spawn_actor(mob);
    ...
    let mut roomcap = spawn_actor(room);
    roomcap.send_cmd(room::GiveMob(mobcap)).unwrap();
    let mut trans = Trans { actor: None };
    {
        trans = roomcap.read_lock(trans).unwrap();
    }
    for line in stdin().lines() {
        let mut line = line.unwrap();
        line.pop_char();
        match line.as_slice() {
            "look" => println!("{}", trans.actor),
            "quit" => break,
